# Combine
Это библиотека от Apple для реактивного программирования представленная в 2019 году. Это позволяет нам получать данные с течением времени, которые могут работать в качестве альтернативы шаблону делегатов или обратным вызовам в блоках замыкания.

В структуре Combine можно выделить основные принципы, которые нам нужно понять, прежде чем погрузиться в нее:
1. [Издатель (Publisher)](#издатель-или-publisher)
    - [Just](#just-только)
    - [Empty](#empty-пустой)
    - [Fail](#fail-неудача)
    - [Future](#future-будущее)
    - [Deferred](#deferred-отложенный)
    - [Sequence](#издатель-из-последовательности)
    - [Subject](#предметы-subjects)
...
1. Подписчик подписывается на издателя, вызывая метод запроса подписки `receive(subscription: Subscription)` передавая подписку в качестве параметра.
...
### Deferred (Отложенный)
Издатель, создающий нового издателя для каждой подписки.
Издатель, который ждет подписки, прежде чем запустить предоставленное закрытие подписки, чтобы создать издателя для нового подписчика.
```swift
...
let sub1 = future.sink(receiveValue: { value in
    print("sub1: \(value)")
}) // Future выполняется, потому что у него есть подписчик

let sub2 = future.sink(receiveValue: { value in
    print("sub2: \(value)")
}) // Future выполняется снова, поскольку получил еще одного подписчика
```
Издатель Deferred каждый раз инициализируется, когда он получает подписчика. Это означает, что новое Future создается каждый раз, когда мы подписываемся на отложенного Deferred издателя. Таким образом, Future все еще работает только один раз и выполняется сразу после его создания, но мы отложим создание Future на более позднее время.
...
### DataTaskPublisher
Возвращает издателя, который обертывает URL session data task для заданного URL-адреса.
Издатель публикует данные по завершении задачи или завершает работу, если задача завершается с ошибкой.
```swift
...
cancellable = URLSession.shared.dataTaskPublisher(for: URL(string: "https://swapi.dev/api/people/1")!)
    .map(\.data)
    .decode(type: StarWarsPeople.self, decoder: JSONDecoder())
    .sink(receiveCompletion: { completion in
        print(completion)
...
| - | - | - | - |
| Определение |  Наблюдаемый объект издатель, который выдает значения с течением времени и который также может быть завершен, когда больше нет доступных значений или когда он столкнулся с ошибкой. | Объект который связывает издателя и подписчика | Объекты или замыкания, используемые для наблюдения за издателем. |
| Protocol | Publisher | Cancellable | Subscriber |
| Eraser Object | struct AnyPublisher | ~~class AnyCancellable~~ | struct AnySubscriber |
| Default | `PassThroughSubject`, `CurrentValueSubject`, `Just`, `Empty`, `Future`, `Fail`, `Sequence`, `@Published`  | `AnyCancellable`, `Set<AnyCancellable>` | `sink(receiveValue:)`, `.assign(to:)`, Custom Subscriber |
